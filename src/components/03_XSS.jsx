    ...
    <div className="w-full mx-auto p-6">
      <div className="app-card-inner bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-lg p-6 max-w-6xl mx-auto">
        <div className="flex items-start gap-4 mb-4">
          <img src={xssImg} alt="XSS reference" className="w-full max-w-full h-auto object-contain rounded-md border" />
          <div className="flex-1">
            <div className="flex items-center gap-3">
              <span className="w-6 h-6 text-sky-500" aria-hidden>📷</span>
              <h2 className="text-2xl font-semibold">{title}</h2>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
              <span className="w-4 h-4 text-amber-500" aria-hidden>⚠️</span> Riesgos, payloads y mitigaciones
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <span className="w-5 h-5 text-gray-600 dark:text-gray-300" aria-hidden>⌨️</span>
